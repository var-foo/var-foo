 ...
 * @package        SensibleUX
 * @author         SensibleUX
 * @copyright      SensibleUX
 * @license        license.txt
 * @version        Release: 1.0
 */

function responsive_posts_shortcode( $atts ) {
	extract( shortcode_atts( array( 'limit' => 5 ), $atts ) );

	return '<ul class="recent-posts">' . wp_get_archives('type=postbypost&limit=' . $limit . '&echo=0') . '</ul>';
}

add_shortcode( 'recent-posts', 'responsive_posts_shortcode' );
